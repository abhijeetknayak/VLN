...
from internnav.dataset.internvla_n1_lerobot_dataset import NavPixelGoalDataset
...
@Evaluator.register('pixel_goal_eval')
class PixelGoalEval(DistributedEvaluator):
    def __init__(self, cfg: EvalCfg):
        ...
        self.dataset = NavPixelGoalDataset(self.processor.tokenizer, data_args)
    ...
    def _run_eval_system2(self) -> tuple:
        ...
        for data in self.dataset:
            # Get data samples where pixel goal is available
            if not isinstance(data['action'], Sequence):
                continue
